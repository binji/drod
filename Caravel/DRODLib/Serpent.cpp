	...
	const int oX, const int oY)	//(in)	Current orientation of head.
{
	if (dx || dy)
	{
		...
		if (dx != 0) 
		{
			...
			default: ASSERTP(false, "Bad orientation.");
			}
		} 
		else
		{
			...
			default: ASSERTP(false, "Bad orientation.(2)");
			}
		}
	...
	CCueEvents &CueEvents)	//(in/out)
{
	...
	switch (tile)
	{
		...
		default: ASSERTP(false, "Bad tile.");
	}
	switch (nGetO(dx, dy))
	{
...
const
{
	...
	for (UINT i = 0; i < num_paths; i++)
	{
		if ((UINT)paths[i].x == this->wX && (UINT)paths[i].y == this->wY)
			continue;
		else if (abs((int)(this->wX - paths[i].x)) != abs((int)(this->wY - paths[i].y)) &&
...
// Assumes a valid serpent.
{
	...
	while (!done)
	{
		...
		case T_SNKT_S: case T_SNKT_W: case T_SNKT_N: case T_SNKT_E:
			...
			break;
		default: ASSERTP(false, "Bad serpent tile.(2)");
		}
	}
	this->tailX = x;
	this->tailY = y;
...
// $Log: Serpent.cpp,v $
// Revision 1.28  2003/10/06 02:41:20  erikh2000
// Added descriptions to assertions.
//
// Revision 1.27  2003/08/02 20:35:55  mrimer
// Refactored serpent behavior into new class CRedSerpent.
//
// Revision 1.26  2003/07/09 11:35:13  schik
// DoesSquareContainObstacle() was declared differently in some
// CMonster subclasses - made them all consistent. (VS.NET warnings)
//
// Revision 1.25  2003/06/20 23:48:03  mrimer
// Brains can now only sense invisible player when within smelling range.
//
// Revision 1.24  2003/06/19 01:53:46  mrimer
// Linux port fixes (committed on behalf of Gerry JJ).
//
// Revision 1.23  2003/06/16 23:36:38  mrimer
// Added prev position vars.  Fixed serpent error.
//
// Revision 1.22  2003/05/24 02:07:21  mrimer
