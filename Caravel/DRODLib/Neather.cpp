							...
							//sound or graphical effects.
{
	...
	if (pGoal.eType != exitroom)
	{
		if (pGoal.eType==strikeorb)
		{
			...
			if ((UINT)pGoal.nX==(this->wX) && (UINT)pGoal.nY==(this->wY+1))
			{
				...
				{
					this->pCurrentGame->pRoom->ActivateOrb(pGoal.nX, pGoal.nY, CueEvents);
					this->bStrikingOrb = true;
					if (this->bLaughWhenOrbHit)
					{
						CueEvents.Add(CID_NeatherLaughing);
	...
	else
	{
		...
		CueEvents.Add(CID_NeatherExitsRoom, this);
		return;
	}
}
...
//true if successful, false otherwise.
{
	...
	nY=this->pCurrentGame->pRoom->dwRoomY;
	switch (nX)
	{
		case 48:
			switch (nY)
			{
				case 2549: bRetVal=GetRoom4Goal(pGoal,CueEvents); break;
				case 2550: bRetVal=GetRoom5Goal(pGoal,CueEvents); break;
				case 2551: bRetVal=GetRoom6Goal(pGoal,CueEvents); break;
				case 2552: bRetVal=GetRoom7Goal(pGoal,CueEvents); break;
				default: ASSERTP(false, "Bad neather room."); break;
			}
			 break;
		case 49:
			switch (nY)
			{
				case 2549: bRetVal=GetRoom3Goal(pGoal,CueEvents);  break;
				case 2550: bRetVal=GetRoom12Goal(pGoal,CueEvents);  break;
				case 2551: bRetVal=GetRoom11Goal(pGoal,CueEvents);  break;
				case 2552: bRetVal=GetRoom8Goal(pGoal,CueEvents);  break;
				default: ASSERTP(false, "Bad neather room."); break;
			}
			break;
		case 50:
			switch (nY)
			{
				case 2549: bRetVal=GetRoom2Goal(pGoal,CueEvents); break;
				case 2550: bRetVal=GetRoom1Goal(pGoal,CueEvents); break;
				case 2551: bRetVal=GetRoom10Goal(pGoal,CueEvents); break;
				case 2552: bRetVal=GetRoom9Goal(pGoal,CueEvents); break;
				default: ASSERTP(false, "Bad neather room."); break;
			}
			break;
		default: ASSERTP(false, "Bad neather room.");; 
   }//...switch (nX)
	
	if (!bRetVal) {pGoal.eType=wait;}
	...
	const UINT sx, const UINT sy)
{
	return !((this->wX==sx && this->wY==sy) ||
			(this->pCurrentGame->swordsman.wX==sx &&
			this->pCurrentGame->swordsman.wY==sy) ||
...
//True if any cue events were added, false if not.
{
	...
	ASSERT(this->pCurrentGame->pRoom->dwRoomX==49 && this->pCurrentGame->pRoom->dwRoomY==2550);
	...
	if (nCommand == CMD_YES)
	{
		...
		else
		{
			...
			else {ASSERTP(false, "Bad state for OnAnswer()(2).");}
		}
		Move(x, y);
...
{
	CueEvents.Add(CID_NeatherScared);

	if (this->pCurrentGame->pRoom->dwRoomX==49 && this->pCurrentGame->pRoom->dwRoomY==2550)
...
//true if successful, false otherwise.
{
	...
	{
		CueEvents.Add(CID_NeatherFrustrated);
	...
	bRetVal=SetGoal(this->wX, this->wY, wait, pGoal);	
	return bRetVal;
}
	...
	while (true)  //Function exits with return.
	{
		switch (this->m_CurrentState)
		{
			...
			case r3_stReleasingRoaches:
			{
				...
				if (!this->pCurrentGame->pRoom->AreMonstersInRect(4, 10, 12, 17))
				{
					CueEvents.Add(CID_NeatherFrustrated);
					this->m_CurrentState=rx_stFleeing;
					break;
				...
				bRoachesAreInLock=this->pCurrentGame->pRoom->AreMonstersInRect(4, 15, 12, 17);
...
//true if successful, false otherwise.
{
	...
	SetGoal(this->wX, this->wY, wait, pGoal);
	while (true)  //Function exits with return.
	{
		switch (this->m_CurrentState)
		{
	...
	while (true)  //Function exits with return.
	{
		switch (this->m_CurrentState)
		{
			...
			case r6_GuardingControlCenter:  //Manning control center.
				...
				if (!this->pCurrentGame->pRoom->AreMonstersInRect(17, 13, 18, 13))
				{
					CueEvents.Add(CID_NeatherFrustrated);
	...
	while (true)  //Function exits with return.
	{
		switch (this->m_CurrentState)
		{
			case rx_stx: //N is manning the controls to trap S.
				...
				if (!this->pCurrentGame->pRoom->AreMonstersInRect(1, 4, 33, 30))
				{
					CueEvents.Add(CID_NeatherFrustrated);
				...
				{
					bInTrap1=true;
					if (this->pCurrentGame->pRoom->IsDoorOpen(7, 28) && !this->pCurrentGame->pRoom->AreMonstersInRect(5, 28, 11, 28))
						{return SetGoal(15, 2, strikeorb, pGoal);}
			    	if (this->pCurrentGame->pRoom->IsDoorOpen(16, 25) && !this->pCurrentGame->pRoom->AreMonstersInRect(16, 23, 16, 27)
			    	 && !this->pCurrentGame->pRoom->AreMonstersInRect(13, 28, 16, 28))	
			    		{return SetGoal(17, 2, strikeorb, pGoal);}
				}
				if (IsInRect(swX, swY, 2, 7, 7, 11))
				{
					bInTrap2=true;
					if (this->pCurrentGame->pRoom->IsDoorOpen(3, 11) && !this->pCurrentGame->pRoom->AreMonstersInRect(3, 9, 3, 13)) 
						{return SetGoal(14, 2, strikeorb, pGoal);}
				}
				if (IsInRect(swX, swY, 16, 5, 23, 12))
				{
					bInTrap3=true;
					if (this->pCurrentGame->pRoom->IsDoorOpen(16, 8) && !this->pCurrentGame->pRoom->AreMonstersInRect(14, 8, 18, 8)) 
						{return SetGoal(16, 2, strikeorb, pGoal);}
					if (this->pCurrentGame->pRoom->IsDoorOpen(24, 8) && !this->pCurrentGame->pRoom->AreMonstersInRect(22, 8, 26, 8))
						{return SetGoal(19, 2, strikeorb, pGoal);}
				}
				if (IsInRect(swX, swY, 19, 25, 25, 28))
				{
					bInTrap4=true;
					if (this->pCurrentGame->pRoom->IsDoorOpen(22, 25) && !this->pCurrentGame->pRoom->AreMonstersInRect(22, 23, 22, 28)) 
				...
				if (IsInRect(swX, swY, 25, 7, 32, 13) || IsInRect(swX, swY, 27, 14, 31, 16))
				{
					...
					if (this->pCurrentGame->pRoom->AreMonstersInRect(25, 7, 32, 13) || this->pCurrentGame->pRoom->AreMonstersInRect(27, 14, 31, 16))
					{
						if (this->pCurrentGame->pRoom->IsDoorOpen(24, 8) && !this->pCurrentGame->pRoom->AreMonstersInRect(22, 8, 26, 8))
							{return SetGoal(19, 2, strikeorb, pGoal);}
						if (this->pCurrentGame->pRoom->IsDoorOpen(27, 16) && !this->pCurrentGame->pRoom->AreMonstersInRect(27, 13, 27, 18)) 
							{return SetGoal(20, 2, strikeorb, pGoal);}
						if (this->pCurrentGame->pRoom->IsDoorOpen(31, 16) && !this->pCurrentGame->pRoom->AreMonstersInRect(31, 14, 31, 18)) 
							{return SetGoal(21, 2, strikeorb, pGoal);}
					}
					//Find nearby serpents to determine door to leave open.
					else if (this->pCurrentGame->pRoom->AreMonstersInRect(29, 17, 32, 24)) //Serpent(s) outside door 3.
					{
						if (this->pCurrentGame->pRoom->IsDoorOpen(24, 8) && !this->pCurrentGame->pRoom->AreMonstersInRect(22, 8, 26, 8))
							{return SetGoal(19, 2, strikeorb, pGoal);}
						if (this->pCurrentGame->pRoom->IsDoorOpen(27, 16) && !this->pCurrentGame->pRoom->AreMonstersInRect(27, 13, 27, 18)) 
					...
					else if (this->pCurrentGame->pRoom->AreMonstersInRect(13, 5, 23, 18)) //Serpent(s) outside door 1.
					{
...
//true if successful, false otherwise.
{
	...
	while (true)  //Function exits with return.
	{
		...
		bIsTrap1Loaded=(this->pCurrentGame->pRoom->AreMonstersInRect(1, 24, 1, 30) || this->pCurrentGame->pRoom->AreMonstersInRect(2, 30, 22, 30));
		...
		bIsTrap2Loaded=(this->pCurrentGame->pRoom->AreMonstersInRect(10, 1, 10, 5) || this->pCurrentGame->pRoom->AreMonstersInRect(11, 5, 19, 5));
		...
		bIsTrap3Loaded=(this->pCurrentGame->pRoom->AreMonstersInRect(13, 23, 21, 23) || this->pCurrentGame->pRoom->AreMonstersInRect(21, 21, 21, 22));
		...
		bIsTrap4Loaded=this->pCurrentGame->pRoom->AreMonstersInRect(16, 11, 19, 17);
		...
		bIsTrap5Loaded=this->pCurrentGame->pRoom->AreMonstersInRect(15, 28, 25, 28);
		...
		bIsTrap6Loaded=this->pCurrentGame->pRoom->AreMonstersInRect(23, 11, 26, 16);
		...
		bIsTrap7Loaded=this->pCurrentGame->pRoom->AreMonstersInRect(23, 18, 26, 22);
		...
		bIsTrap8Loaded=this->pCurrentGame->pRoom->AreMonstersInRect(31, 8, 32, 15);
		...
		bIsTrap9Loaded=this->pCurrentGame->pRoom->AreMonstersInRect(31, 23, 33, 28);
	...
	while (true)  //Function exits with return.
	{
		switch (this->m_CurrentState)
		{
				...
				if (swX<19) //Swordsman is on west side.
				{
					switch (swY)
					{
						...
						case 7:
							...
							else {ASSERTP(false, "Bad swX.");}													
						default: ASSERTP(false, "Bad swY(2).");													
					}
				}
				else //Swordsman is on east side. Maybe he thinks he'll head off Neather, but he won't.
				{
					switch (swY)
					{
						...
						case 7:
							...
							else {ASSERTP(false,"Bad swX(3)");}													
						default: ASSERTP(false,"Bad swY(3)");													
					}
				}
				return SetGoal(x, y, move, pGoal);
			...
			case rx_stEight: //Determine what to do now that Swordsman is trapped.
				if (!(this->pCurrentGame->pRoom->GetOSquare(26, 2)==T_PIT)) {this->m_CurrentState=rx_sta; break;} //Leave room through north exit.
				if (!(this->pCurrentGame->pRoom->GetOSquare(3, 6)==T_PIT)) {this->m_CurrentState=rx_stA; break;} //Leave room through west exit.
...
//
// Revision 1.25  2003/06/19 01:53:45  mrimer
// Linux port fixes (committed on behalf of Gerry JJ).
//
// Revision 1.24  2003/06/03 06:20:27  mrimer
// Fixed a bug.
//
// Revision 1.23  2003/05/25 22:46:28  erikh2000
// Revised #includes:
// 1. Consistent use of <> instead of "" for backendlib and frontendlib.
// 2. MID constants separated from MessageIDs.h to reduce dependencies during builds.
//
// Revision 1.22  2003/04/06 03:57:01  schik
// Ported to SGI.
// All filenames in CFiles and elsewhere are now in Unicode if the platform supports it.
//
// Revision 1.21  2002/12/22 01:23:13  mrimer
// Removed some parameters to GetGoalDirectedMovement().
// Revised swordsman vars.
//
// Revision 1.20  2002/11/18 17:56:17  mrimer
// Fixed bad rect once and for all.
//
// Revision 1.19  2002/09/14 21:18:23  mrimer
// Refactored movement code into CMonster.
//
// Revision 1.18  2002/08/31 01:06:58  erikh2000
// Fixed a bad rect causing 'Neather to be less smart.
//
// Revision 1.17  2002/08/29 22:00:39  mrimer
// Modified Process(), removing return type.  Some tweaking.
//
// Revision 1.16  2002/08/28 20:24:46  mrimer
// Added sound effect logic.
// Fixed a room 3 bug.
//
// Revision 1.15  2002/07/22 17:32:28  mrimer
// Added bStrikingOrb.
//
// Revision 1.14  2002/07/16 18:21:18  mrimer
// Fixed goal destination logic for orb striking.
// Fixed path selection logic in room #7.
// Fixed Neather spared movement in room #12.
// Fixed other minor bugs in some rooms.
//
// Revision 1.13  2002/06/21 04:42:38  mrimer
// Revised includes.
//
// Revision 1.12  2002/06/20 04:07:59  erikh2000
...
//
// Revision 1.9  2002/02/27 02:25:33  erikh2000
// Changed to Process(), OnAnswer(), GetRoom5Goal(), GetRoom6Goal(), GetRoom8Goal() to fix bugs found in build 3.  (Committed on behalf of j_wicks.)
//
// Revision 1.8  2002/02/07 23:27:39  erikh2000
...
//
// Revision 1.5  2001/10/21 00:26:41  erikh2000
// Commented out declarations to get rid of warnings.
//
// Revision 1.4  2001/10/20 05:44:58  erikh2000
// Commented out routines that will get rewritten later as part of CMonster-derived classes.
//
