...
const
{
	...
	for (UINT dir = 0; dir < ORIENTATION_COUNT; ++dir)
	{
			 ...
			 !DoesArrowPreventMovement(ndx, ndy)))
		{
         ...
         else if (abs((int)(x - this->pCurrentGame->swordsman.wSwordX)) <= 1 &&
					abs((int)(y - this->pCurrentGame->swordsman.wSwordY)) <= 1)
            score = 5000L;	//Near sword -- low score
			else
			{
				//Closer is better
            const float fDist = DistanceToSwordsman(x, y);
            score = fDist > 900.0f ? 0 : 90000L - (DWORD)(100.0f * fDist);
				ASSERT(score <= 90000L);
			}
		}
		if (score > bestscore)
...
// $Log: Goblin.cpp,v $
// Revision 1.27  2003/07/23 19:47:25  mrimer
// Fixed assertion firing when brain-directed goblin on force arrow can't move.
//
// Revision 1.26  2003/07/19 02:28:02  mrimer
// Fixed an assertion (although there might be a bug in pathmap generation).
//
// Revision 1.25  2003/07/10 15:55:00  mrimer
// WCHAR string port fixes (committed on behalf of Gerry JJ).
//
// Revision 1.24  2003/06/20 23:48:03  mrimer
// Brains can now only sense invisible player when within smelling range.
//
// Revision 1.23  2003/06/19 01:53:45  mrimer
