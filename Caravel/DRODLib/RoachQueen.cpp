		...
		for (int y = -1; y <= 1; y++)
		{
			for (int x = -1; x <= 1; x++)
			{
					...
					this->pCurrentGame->pRoom->GetTSquare(ex, ey) == T_EMPTY)
				{
					...
					if (wOSquare == T_FLOOR	|| wOSquare == T_CHECKPOINT)
					{
						//Place an egg.
...
// Revision 1.26  2004/08/08 21:26:57  mrimer
// Fixed bug: eggs not spawning on checkpoints.
//
// Revision 1.25  2003/08/06 15:44:32  erikh2000
