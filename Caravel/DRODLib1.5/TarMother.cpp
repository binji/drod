							...
							//sound or graphical effects.
{
	...
	if (pCurrentGame->wSpawnCycleCount % TURNS_PER_CYCLE == TURNS_PER_CYCLE-2)
	...
	if (pCurrentGame->wSpawnCycleCount % TURNS_PER_CYCLE == TURNS_PER_CYCLE-1)
		if (TarCanFindSwordsman())
			if (!CueEvents.HasOccurred(CID_TarGrew)) 
				CueEvents.Add(CID_TarGrew);
}

//*****************************************************************************
bool CTarMother::TarCanFindSwordsman() const
//Returns: true if tar mother OR a tar baby can sense the swordsman.
{
	if (CanFindSwordsman()) return true;
	
	//Check all tar babies in room.
