...
//*****************************************************************************
CCurrentGame::CCurrentGame(void)
//Constructor.
	: pRoom(NULL)
	, pLevel(NULL)
	, pHold(NULL)
...
//*****************************************************************************
bool CCurrentGame::IsLevelStart(void)
//Is the current game at the beginning of a level.
{
	...
	return (this->pRoom->dwRoomX == dwStartRoomX &&
			this->pRoom->dwRoomY == dwStartRoomY);
}
...
//*****************************************************************************
DWORD CCurrentGame::EndDemoRecording(void)
...
//0 if no commands recorded.
{
	...
	if (this->DemoRecInfo.wBeginTurnNo < this->wTurnNo)
		...
		WriteCurrentRoomDemo(this->DemoRecInfo);
...
//*****************************************************************************
DWORD CCurrentGame::GetChecksum(void)
							...
							//		data on return.
{
	...
	if (UnansweredQuestions.size())
		ProcessUnansweredQuestions(nCommand, UnansweredQuestions, CueEvents);
	else
	{
	...
	AddQuestionsToList(CueEvents, UnansweredQuestions);
...
//*****************************************************************************
bool CCurrentGame::SetSwordsmanToEastExit(void)
//Move swordsman to an exit along east column.
{
	...
	for (UINT wRowOffset = 0; ; ++wRowOffset)
	{
		...
		if (bNorthInBounds)
		{
			if (!this->pRoom->DoesSquareContainSwordsmanObstacle(
				this->pRoom->wRoomCols - 1, this->swordsman.wY - wRowOffset, E))
			{
...
//*****************************************************************************
bool CCurrentGame::SetSwordsmanToWestExit(void)
//Move swordsman to an exit along west column.
{
	...
	for (UINT wRowOffset = 0; ; ++wRowOffset)
	{
		...
		if (bNorthInBounds)
		{
			if (!this->pRoom->DoesSquareContainSwordsmanObstacle(0, 
					this->swordsman.wY - wRowOffset, W))
			{
...
//*****************************************************************************
bool CCurrentGame::SetSwordsmanToNorthExit(void)
//Move swordsman to an exit along north row.
{
	...
	for (UINT wColOffset = 0; ; ++wColOffset)
	{
		...
		{
			if (!this->pRoom->DoesSquareContainSwordsmanObstacle(
				this->swordsman.wX - wColOffset, 0, N))
			{
...
//*****************************************************************************
bool CCurrentGame::SetSwordsmanToSouthExit(void)
//Move swordsman to an exit along south row.
{
	...
	for (UINT wColOffset = 0; ; ++wColOffset)
	{
		...
		{
			if (!this->pRoom->DoesSquareContainSwordsmanObstacle(
				this->swordsman.wX - wColOffset, this->pRoom->wRoomRows - 1, S))
			{
...
//*****************************************************************************
void CCurrentGame::SaveToContinue(void)
//Save the current player's game to the continue slot.
{
	...
	CDb db;
	const DWORD dwContinueID = db.SavedGames.FindByContinue();
...
//*****************************************************************************
void CCurrentGame::SaveToLevelBegin(void)
//Saves the current game to the level-begin slot for this level.
{
	...
	CDb db;
	const DWORD dwExistingSavedGameID = db.SavedGames.FindByLevelBegin(
...
//*****************************************************************************
void CCurrentGame::SaveToRoomBegin(void)
//Saves the current game to the begin-room slot for this room.
{
	...
	CDb db;
	const DWORD dwExistingSavedGameID = db.SavedGames.FindByRoomBegin(
			this->pRoom->dwRoomID);
	if (dwExistingSavedGameID)	//Yes.
...
//*****************************************************************************
void CCurrentGame::SaveToCheckpoint(void)
//Saves the current game to the checkpoint slot for this room/checkpoint.
{
	...
	CDb db;
	const DWORD dwExistingSavedGameID = db.SavedGames.FindByCheckpoint(
...
//*****************************************************************************
bool CCurrentGame::IsCurrentRoomConquered(void) const
{
	return CDbSavedGame::ConqueredRooms.IsIDInList(this->pRoom->dwRoomID);
}

//*****************************************************************************
bool CCurrentGame::IsCurrentRoomExplored(void) const
{
...
//Set conquered/explored status of rooms from all saved games in the current level.
//
//Params:
	const DWORD dwCurrentRoomID)
{
	...
	c4_View ConqueredRoomsView, ExploredRoomsView;
	...
	for (DWORD dwSavedGamesI = 0; dwSavedGamesI < dwSavedGamesCount; ++dwSavedGamesI)
	{
		...
		//Set conquered rooms from the saved game for the current room.
		if (dwLoopRoomID == dwCurrentRoomID)
		{
			this->ConqueredRooms.Clear();
			ConqueredRoomsView = p_ConqueredRooms(SavedGamesView[dwSavedGamesI]);
			dwRoomCount = ConqueredRoomsView.GetSize();
			for (dwI = 0; dwI < dwRoomCount; ++dwI)
			{
				dwRoomID = p_RoomID(ConqueredRoomsView[dwI]);
...
//True if successful, false if not.
{
	...
	if (IsDemoRecording() &&
			!EndDemoRecording())
		CFiles::AppendErrorLog("Failed to save a demo when recording ended.\r\n");
	...
	//Save to level-begin and room-begin slots.
	{
		...
		if ((this->dwAutoSaveOptions & ASO_LEVELBEGIN)==ASO_LEVELBEGIN &&
				!db.SavedGames.FindByLevelBegin(this->pLevel->dwLevelID)) 
			SaveToLevelBegin();
		if ((this->dwAutoSaveOptions & ASO_ROOMBEGIN)==ASO_ROOMBEGIN &&
				!db.SavedGames.FindByRoomBegin(this->pRoom->dwRoomID)) 
			SaveToRoomBegin();
	}
...
//True if successful, false if not.
{
	...
	//Saving disabled during testing.
	this->dwAutoSaveOptions = ASO_NONE;
...
//True if successful, false if not.
{
	...
	if (!bSuccess) goto Cleanup;
	this->swordsman.wX = CDbSavedGame::wStartRoomX;
	this->swordsman.wY = CDbSavedGame::wStartRoomY;
	...
	else
	{
		...
		SetMembersAfterRoomLoad(IgnoredCueEvents);
		...
		CDb db;
		const DWORD dwLatestSavedGame = db.SavedGames.FindByRoomLatest(
				this->pRoom->dwRoomID);
		if (dwLatestSavedGame)
		{
			CDbSavedGame *pSavedGame = db.SavedGames.GetByID(dwLatestSavedGame);
							...
							//		into the room.
{
	...
	{
		LoadFromSavedGame(dwLastCheckpointSavedGameID_,CueEvents,false);
		ASSERT(this->pRoom);
		ASSERT(this->pRoom->GetOSquare(this->swordsman.wX, this->swordsman.wY)==T_CHECKPOINT);
	}
	else
...
//*****************************************************************************
bool CCurrentGame::IsCurrentLevelComplete(void) const
//For the current level, has the player conquered enough rooms for the 
//completion doors to open?
//
//Returns:
//True if so, false if not.
{
	const DWORD dwConquered = CDbSavedGame::ConqueredRooms.GetSize();
	const DWORD dwNeeded = (DWORD) pLevel->wRoomsNeededToComplete;
	return (dwConquered >= dwNeeded);
}
...
//*****************************************************************************
void CCurrentGame::SetCurrentRoomConquered(void)
...
//*****************************************************************************
void CCurrentGame::SetCurrentRoomExplored(void)
...
//*****************************************************************************
bool CCurrentGame::SwordsmanIsDying(void) const
//Returns: whether the current state indicates the the swordsman is dying
{
	return (pRoom->GetMonsterAtSquare(swordsman.wX,swordsman.wY) != NULL);
}
...
//stay loaded.
{
	CDbRoom *pOldRoom = this->pRoom;

	//Load new room.
	this->pRoom = this->pLevel->GetRoomAtCoords(dwRoomX, dwRoomY);
	if (this->pRoom)
	{
		//Free the old room and return successful.
		if (pOldRoom) delete pOldRoom;
		return true;
	}
	else
	{
		//Set the current room back to old room and return failure.
		this->pRoom = pOldRoom;
		return false;
	}
}
...
//*****************************************************************************
void CCurrentGame::FreezeCommands(void)
...
//*****************************************************************************
void CCurrentGame::UnfreezeCommands(void)
...
//*****************************************************************************
bool CCurrentGame::WalkDownStairs(void)
...
const
{
	CMonsterMessage *pMessage = reinterpret_cast<CMonsterMessage *>
			(CueEvents.GetFirstPrivateData(CID_MonsterSpoke));
	while (pMessage)
	{
		if (pMessage->eType == MMT_YESNO)
			QuestionList.push_back(*pMessage);
		pMessage = reinterpret_cast<CMonsterMessage *>(CueEvents.GetNextPrivateData());
	}
}
...
//unexpected state, false if not.
{
	...
	for (UINT wCommandI = 0; wCommandI < wCommandCount; ++wCommandI)
	{
		ASSERT(pCommand);
	...
	CCueEvents &CueEvents)						//(out)		Adds cue events as appropriate.
{
	...
	if (iQuestion->eType == MMT_YESNO)
	{
		...
		CMonster *pStabbedMonster = reinterpret_cast<CMonster *>(
		...
		while (pStabbedMonster )
		{
			...
			pStabbedMonster = reinterpret_cast<CMonster *>(
							...
							//		into the room.
{
	...
	CDb db;
	this->dwPlayerID = db.GetPlayerID();
	...
	//Remove a crumbly wall underneath the swordsman if it exists.
	if (this->pRoom->GetOSquare(this->swordsman.wX, this->swordsman.wY)==T_WALL_B)
			this->pRoom->DestroyCrumblyWall(this->swordsman.wX, this->swordsman.wY, this->swordsman.wO, CueEvents);
	...
	dwLastCheckpointSavedGameID = 0L;
}
							...
							//		data on return.
{
	ASSERT(this->swordsman.bIsPlacingMimic);
 	...
 	if (nCommand==CMD_WAIT) // Create a new Mimic
 	{
 		...
 		{
 			this->swordsman.bIsPlacingMimic=false;
 			CMimic *pMimic = dynamic_cast<CMimic *>(pRoom->AddNewMonster(M_MIMIC,
	...
	//Look for obstacles and set dx, dy accordingly.
 	else if (dx != 0 || dy != 0)
 	{
 		...
 		if (!pRoom->IsValidColRow(this->swordsman.wMimicCursorX + dx, this->swordsman.wMimicCursorY + dy))
 		{
 			...
 			//in that direction.  Just treat it as an obstacle.
 			dx = dy = 0;
 		}
 		else
 		{
								...
								//		that mimic's sword.
{
	...
	if (this->pRoom->GetOSquare(wSX, wSY) == T_WALL_B) 
		this->pRoom->DestroyCrumblyWall(wSX, wSY, this->swordsman.wO, CueEvents);
							...
							//		data on return.
{
	...
	const bool bWasOnScroll = (pRoom->GetTSquare(this->swordsman.wX, this->swordsman.wY)==T_SCROLL);
	...
	if (!bWasOnScroll && wNewTSquare == T_SCROLL)
	{
	...
	switch(wNewTSquare)
	{
		...
		case T_POTION_I:	//Invisibility potion.
			this->swordsman.bIsVisible = false;
			pRoom->Plot(this->swordsman.wX, this->swordsman.wY, T_EMPTY);
			CueEvents.Add(CID_DrankPotion);
		break;
...
//unsuccessful (ProcessSwordsman() should keep swordsman in current room).
{
	...
	if (this->bIsDemoRecording)
	{
		const DWORD dwDemoID = WriteCurrentRoomDemo(this->DemoRecInfo);
	...
	if (bConquered)
	{
		...
		//If enough rooms have been conquered, then level is completed.
		if (this->ConqueredRooms.GetSize() == this->pLevel->wRoomsNeededToComplete)
		{
	...
	//Leave the room.
	const DWORD dwExploredCount = this->ExploredRooms.GetSize();
	const UINT wExitDirection = GetRoomExitDirection(wMoveO);
	if (LoadNewRoomForExit(wExitDirection, CueEvents))
	{
		CDb db;
		if ((this->dwAutoSaveOptions & ASO_ROOMBEGIN)==ASO_ROOMBEGIN &&
				this->ExploredRooms.GetSize() > dwExploredCount &&
				!db.SavedGames.FindByRoomBegin(this->pRoom->dwRoomID))
			SaveToRoomBegin(); //Save game for newly explored room.
		CueEvents.Add(CID_ExitRoom, new CAttachableWrapper<UINT>(wExitDirection), true);
		return true;
	}
	return false; //Failed to load new room.
}
	...
	CCueEvents &CueEvents)	//(out)	Events added to it.
{
	...
	for (UINT i = 0; i < pRoom->wExitCount; i++)
	{
		const CExitData &stairs = pRoom->parrExits[i];
...
//***************************************************************************************
bool CCurrentGame::WasRoomConqueredOnThisVisit(void) 
...
//from the north end of the room.  Swordsma
//
//Params:
	CCueEvents &CueEvents)	//(out)	Cue events generated by swordsman's first step 
							//		into the room.
...
//***************************************************************************************
void CCurrentGame::ClearRoomLists(void)
...
//***************************************************************************************
DWORD CCurrentGame::WriteCurrentRoomConquerDemo(void)
...
//***************************************************************************************
DWORD CCurrentGame::WriteCurrentRoomDieDemo(void)
...
//***************************************************************************************
DWORD CCurrentGame::WriteCurrentRoomHighlightDemo(void)
...
{	
	...
	//This call does the real work.
	return WriteCurrentRoomDemo(dri, true);
}
...
//Params:
	DEMO_REC_INFO &dri,	//(in/out)	Receives with members set with info needed to
						...
						//			demo.
	bool bHidden)		//(in)		Make the demo hidden?  Default is false.
//
//Returns:
//DemoID of new Demos record.
{
	...
	//Get a new demo and set it's properties.
	...
	pDemo->DescriptionText.Bind(dri.dwDescriptionMessageID);
	if (dri.dwPrevDemoID)
	{
		WSTRING wstrNewDescription = (const WCHAR *)pDemo->DescriptionText;
		if (dri.dwPrevDemoID == dri.dwFirstDemoID) //2nd demo.
			wstrNewDescription += L" (2)";
		else //3rd or later demo.
		{
			//Parse number from description text.
			WSTRING wstrNum;
			WSTRING::iterator iNumStart = NULL, iNumStop = NULL;
			for (WSTRING::iterator iSeek = wstrNewDescription.end() - 1;
				iSeek != wstrNewDescription.begin(); --iSeek)
			{
				if (*iSeek == L')') {iNumStop = iSeek; continue;}
				if (*iSeek == L'(') {iNumStart = iSeek + 1; break;}
				wstrNum += *iSeek;
			}
			int nNum = _wtoi(wstrNum.begin());

			//Parsing error or previous call didn't append "(n)".
			ASSERT(nNum != 0);
			ASSERT(iNumStart != NULL && iNumStop != NULL);
		...
		pDemo->DescriptionText = wstrNewDescription.begin();
	...
	//so that it's NextDemoID field will point to this new demo record.
	if (dri.dwPrevDemoID)
	{
		CDbDemos demos;
		CDbDemo *pPrevDemo = demos.GetByID(dri.dwPrevDemoID);
		...
		if (pPrevDemo->dwDemoID == dri.dwFirstDemoID) //First demo.
		{
			...
			wstrNewDescription += L" (1)";
			pPrevDemo->DescriptionText = wstrNewDescription.begin();
		}
		pPrevDemo->Update();
		delete pPrevDemo;
	}
...
//***************************************************************************************
void CCurrentGame::SetSwordsmanToRoomStart(void)
...
//***************************************************************************************
void CCurrentGame::SetRoomStartToSwordsman(void)
...
//***************************************************************************************
void CCurrentGame::CalcPathMaps(void)
...
//***************************************************************************************
void CCurrentGame::CreatePathMaps(void)
...
//***************************************************************************************
bool CCurrentGame::IsSwordsmanTired(void)
...
// Revision 1.1  2003/02/25 00:01:18  erikh2000
// Initial check-in.
//
