 ...
 *
 * Contributor(s):
 * Michael Welsh Duggan (md5i), John Wm. Wicks (j_wicks)
 *
 * ***** END LICENSE BLOCK ***** */
...
{
	ASSERT(abs(dxFirst) <= 1);
	ASSERT(abs(dyFirst) <= 1);
...
const
{
	...
	{
		ASSERT(false);	//(wCol,wRow) must be valid.
		return true;
	}
...
//
//Params:
	DWORD dwMessageID,		//(in)	Message to say.  One of the MID_* constants.
...
const
{
	...
	CMonsterMessage *pNewMessage = 
			new CMonsterMessage(MMT_OK, dwMessageID, const_cast<CMonster *>(this));
...
//
//Params:
	DWORD dwMessageID,		//(in)	Message for question to ask.  One of the MID_* constants.
	CCueEvents &CueEvents)	//(out)	One new CID_MonsterSpoke cue event will be added.
const
{
	ASSERT(dwMessageID);
	...
	//Create the monster message.
	CMonsterMessage *pNewMessage = 
		new CMonsterMessage(MMT_YESNO, dwMessageID, const_cast<CMonster *>(this));
